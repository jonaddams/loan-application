...
export default function TestApiPage() {
	...
	useEffect(() => {
		const loadSampleImages = async () => {
			try {
				const images = await getBankStatementImages();
				setSampleImages(images);

				const initialResults: ProcessingResult[] = images.map((filename) => ({
					filename,
					imagePath: `/documents/bank-statements/${filename}`,
					status: "pending",
				}));
	...
	const handleTestApi = async () => {
		...
		try {
			...
			// Step 2: Find bank statement template
			const bankStatementTemplate = predefinedTemplates.find(
				(template) =>
					template.identifier === "bank_statement" ||
					template.name.toLowerCase().includes("bank") ||
					template.name.toLowerCase().includes("statement"),
			);

			if (!bankStatementTemplate) {
				throw new Error(
					"Bank statement template not found in predefined templates",
				);
			}

			console.log("Found bank statement template:", bankStatementTemplate);

			setSelectedTemplate(bankStatementTemplate);

			// Step 3: Register the template to get a componentId
			console.log("Registering bank statement template...");
			const registerResponse = await registerComponent([
				{
					name: bankStatementTemplate.name,
					fields: bankStatementTemplate.fields.map((field) => ({
						...field,
						validationMethod:
							field.validationMethod || "PostalAddressIntegrity",
					})),
					identifier: bankStatementTemplate.identifier,
					semanticDescription: bankStatementTemplate.semanticDescription,
				},
			]);
			...
			for (let i = 0; i < sampleImages.length; i++) {
				...
				const imagePath = `/documents/bank-statements/${filename}`;
			...
			<div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
				...
				<div className="text-center mb-8">
					<h1 className="text-3xl font-bold text-gray-900 sm:text-4xl">
						API Test - Bank Statement Processing
					</h1>
					<p className="mt-4 text-lg text-gray-600">
						Test the Nutrient AI Document Processing API with sample bank
						statement documents
					</p>
				</div>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-gray-600">
								Test processing {sampleImages.length} sample bank statement
								documents
							</p>
						</div>
						<button
							...
							disabled={loading}
							className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
						>
							{loading ? (
								<>
								...
								<div className="relative h-64 w-full bg-gray-100 rounded-lg overflow-hidden">
									{isPdfFile(result.filename) ? (
										...
										<Image
											...
											alt={`Bank Statement - ${result.filename}`}
											fill
