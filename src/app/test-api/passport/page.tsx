...
export default function TestApiPage() {
	...
	const handleTestApi = async () => {
		...
		try {
			...
			// Step 2: Find passport template
			const passportTemplate = predefinedTemplates.find(
				(template) =>
					template.identifier === "passport" ||
					template.name.toLowerCase().includes("passport"),
			);

			if (!passportTemplate) {
				throw new Error("Passport template not found in predefined templates");
			}

			console.log("Found passport template:", passportTemplate);

			setSelectedTemplate(passportTemplate);

			// Step 3: Register the template to get a componentId
			console.log("Registering passport template...");
			const registerResponse = await registerComponent([
				{
					name: passportTemplate.name,
					fields: passportTemplate.fields.map((field) => ({
						...field,
						validationMethod:
							field.validationMethod || "PostalAddressIntegrity",
					})),
					identifier: passportTemplate.identifier,
					semanticDescription: passportTemplate.semanticDescription,
				},
			]);
			...
			for (let i = 0; i < sampleImages.length; i++) {
				...
				const imagePath = `/documents/passports/${filename}`;
			...
			<div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
				...
				<div className="text-center mb-8">
					<h1 className="text-3xl font-bold text-gray-900 sm:text-4xl">
						API Test - Passport Processing
					</h1>
					<p className="mt-4 text-lg text-gray-600">
						Test the Nutrient AI Document Processing API with sample passport
						images
					</p>
				</div>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-gray-600">
								Test processing {sampleImages.length} sample passport images
							</p>
						</div>
						<button
							type="button"
							...
							disabled={loading}
							className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
						>
							{loading ? (
								<>
